...
test("audits Codex user and repository skill scopes without exposing absolute paths", async (context) => {
  ...
  assert.equal(result.pressure.budgetChars, 8000);
  assert.equal(result.pressure.status, "within-8k-ceiling");
...
test("skills CLI emits deterministic JSON and exposes the audit in help", async (context) => {
  ...
  assert.match(help[0], /instructree skills <cwd>/);
  assert.equal(JSON.parse(first[0]).skills[0].name, "repo-skill");
});
