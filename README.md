...
```bash
npx --yes github:kotobuki09/instructree#v0.11.0
```

Add the same audit to every pull request without copying workflow YAML:

```bash
npx --yes github:kotobuki09/instructree#v0.11.0 init
```
...
```bash
npm install --global github:kotobuki09/instructree#v0.11.0
instructree
```
...
jobs:
  instructree:
    ...
    steps:
      ...
      - uses: kotobuki09/instructree@v0.11.0
        with:
          strict: true
```
