...
```bash
npx --yes github:kotobuki09/instructree#v0.16.0
```

Add the same audit to every pull request without copying workflow YAML:

```bash
npx --yes github:kotobuki09/instructree#v0.16.0 init
```

Send findings to GitHub code scanning with one command:

```bash
npx --yes github:kotobuki09/instructree#v0.16.0 init --code-scanning
```
...
```bash
npx --yes github:kotobuki09/instructree#v0.16.0 explain src/api/client.ts --client codex
```
...
```bash
npm install --global github:kotobuki09/instructree#v0.16.0
instructree
```
...
```bash
gh skill install kotobuki09/instructree instructree@v0.16.0 --agent codex --scope user
```
...
jobs:
  instructree:
    ...
    steps:
      ...
      - uses: kotobuki09/instructree@v0.16.0
        with:
          strict: true
```
